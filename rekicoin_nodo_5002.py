...
# Creando una dirección para el Nodo en Puerto 5002
...
def mine_block():
    ...
    blockchain.add_transaction(sender=node_address, receiver='Iker', amount=1)
...
def is_valid():
    ...
    if is_valid:
        ...
        response = {'message':'NOT OK, Blokchain is not valid!'}
    return jsonify(response), 200
...
def add_transaction():
    ...
    if not all (key in json for key in transaction_keys):
        return 'Any element of the transaction is missing.', 400
...
def connect_node():
    ...
    response = {'message':'All nodes are now connected. The Rekicoin blockchain contains the following nodes:',
                'total_nodes':list(blockchain.nodes)}
    return jsonify(response), 201


# Reemplaazando la cadna por la más larga
